"""
Hybrid New World Screwworm-Inspired Optimization (H-NWSO)
"""
...
@dataclass
class HNWSOConfig:
    """Frozen parameterization used in the reported H-NWSO experiments"""

    pop_size: int = 40
    max_evals: Optional[int] = None
    seed: Optional[int] = 42
    beta_levy: float = 1.5
    p_disp_max: float = 0.30
    p_disp_min: float = 0.05
    lambda0: float = 0.10
    q_top: float = 0.25
    p_ov: float = 0.70
    m_min: int = 2
...
def h_nwso_optimize(
    ...
    lb: Array,
    ub: Array,
    config: Optional[HNWSOConfig] = None,
    g_ineq: Optional[Constraint] = None,
    h_eq: Optional[Constraint] = None,
    return_history: bool = False) -> dict[str, object]:

    """Minimize an objective with H-NWSO and DE recombination.

    H-NWSO retains the same normalized coherent penetration operator as NWSO.
    Its performance-oriented additions are top-site oviposition, Levy scouts,
    p-best guides, capped adult-larva competition, and current-to-pbest/1/bin
    differential-evolution recombination.
    """

    cfg = config or HNWSOConfig()
    ...
    adults = lower + rng.random((population_size, dimension)) * span
    ...
    while evaluator.remaining > 0:
        ...
        dispersion_probability = ( cfg.p_disp_max * (1.0 - progress) + cfg.p_disp_min * progress)
        dispersion_scale = cfg.lambda0 * (1.0 - progress)
        egg_scale = cfg.sigma0 * (1.0 - progress) + 1.0e-12
        elites, elite_values = save_elites( adults, adult_values, cfg.elitism)

        # Stage 1: adults disperse by bounded Levy flights.
        dispersed = np.flatnonzero( rng.random(population_size) < dispersion_probability)[: evaluator.remaining]
        if dispersed.size:
            trials = clip_to_bounds(
                adults[dispersed]
                + dispersion_scale * levy_flight((dispersed.size, dimension), cfg.beta_levy, rng) * span,
                lower,
                upper)
            adults[dispersed] = trials
            adult_values[dispersed] = evaluator.evaluate(trials)
        restore_elites(adults, adult_values, elites, elite_values)

        # Stage 2: top sites produce larvae; some larvae become Levy scouts
        site_count = max( 1, min( population_size, int(round(cfg.q_top * population_size))))
        site_indices = np.argsort(adult_values)[:site_count]
        sites = adults[site_indices]

        pbest_count = max(2, min( population_size, int(round(cfg.pbest_frac * population_size))))
        pbest_indices = np.argsort(adult_values)[:pbest_count]
        pbest = adults[pbest_indices]
        pbest_values = adult_values[pbest_indices]

        larval_batches: list[Array] = []
        for rank, site in enumerate(sites):
            if rng.random() > cfg.p_ov:
                continue
            rank_weight = (1.0 if site_count == 1
                else 1.0 - rank / (site_count - 1))

            larva_count = int( round( cfg.m_min + (cfg.m_max - cfg.m_min) * rank_weight))
            larva_count = max(cfg.m_min, min(cfg.m_max, larva_count))
            batch = clip_to_bounds( site + rng.normal( 0.0, egg_scale,size=(larva_count, dimension)) * span,
                lower,
                upper)

            scout_count = min(larva_count, int(round(cfg.p_scout * larva_count)))
            if scout_count:
                scout_indices = rng.choice(
                    larva_count,
                    size=scout_count,
                    replace=False)

                batch[scout_indices] = clip_to_bounds(
                    batch[scout_indices]
                    + cfg.scout_scale
                    * levy_flight((scout_count, dimension), cfg.beta_levy, rng) * span,
                    lower,
                    upper)
            larval_batches.append(batch)

        larvae = ( np.vstack(larval_batches)
            if larval_batches
            else np.empty((0, dimension)))

        # Stage 3: cap the pool and penetrate toward sampled p-best guides
        if larvae.size and evaluator.remaining:
            pool_limit = max( population_size, int( round( population_size * (1.0 + cfg.pool_factor))))
            larva_capacity = min(
                larvae.shape[0],
                max(0, pool_limit - population_size),
                evaluator.remaining)

            if larva_capacity:
                selected_larvae = rng.choice(
                    larvae.shape[0],
                    size=larva_capacity,
                    replace=False)
                larvae = larvae[selected_larvae]

            diversity = population_diversity( adults, adult_values, lower, upper )
            guide_indices = rng.integers( 0, pbest.shape[0], size=larvae.shape[0])
            guides = pbest[guide_indices]
            guide_values = pbest_values[guide_indices]
            ...
            add_penetration_diagnostics(penetration_totals, batch_diagnostics)

            # Stage 4: terminal larvae compete with the current adults
            if larva_capacity:
        ...
        # Stage 5: current-to-pbest/1/bin recombination refines adults
        if cfg.p_de > 0 and evaluator.remaining:
            pbest_count = max( 2, min( population_size, int(round(cfg.pbest_frac * population_size))))

            pbest = adults[np.argsort(adult_values)[:pbest_count]]
            target_count = min( max(1, int(round(cfg.p_de * population_size))), population_size, evaluator.remaining)
            targets = rng.choice( population_size, size=target_count, replace=False)
            trials = np.empty((target_count, dimension), dtype=float)

            for row, target in enumerate(targets):
                candidates = np.delete( np.arange(population_size), target)
                first, second = rng.choice( candidates, size=2, replace=False)
                guide = pbest[int(rng.integers(pbest_count))]
                mutant = (
                    adults[target]
                    + cfg.de_f * (guide - adults[target])
                    + cfg.de_f * (adults[first] - adults[second]))
                crossover = rng.random(dimension) < cfg.de_cr
                crossover[int(rng.integers(dimension))] = True
                trials[row] = np.where( crossover, mutant, adults[target])

            trials = clip_to_bounds(trials, lower, upper)
            trial_values = evaluator.evaluate(trials)
            improved = trial_values <= adult_values[targets]
            adults[targets[improved]] = trials[improved]
            adult_values[targets[improved]] = trial_values[improved]

        # Stage 6: stagnation and low diversity activate adult emergence
        current_best = adults[int(np.argmin(adult_values))]
        diversity = population_diversity( adults, adult_values, lower, upper)
        stall_limit = max( population_size, int(math.ceil(cfg.stall_fraction * max_evaluations)))
        is_stalled = ( evaluator.evaluations - evaluator.last_improvement_evaluation >= stall_limit )
        emergence_is_active = (
            is_stalled
        ...
        if emergence_is_active:
            ...
            if cfg.emergence_mode == "uniform":
                emerged = lower + rng.random((emergence_count, dimension)) * span
            else:
                emerged = ( current_best + rng.normal( 0.0, cfg.emergence_sigma, size=(emergence_count, dimension)) * span)
    ...
    return build_result( evaluator, cycles, return_history, penetration_totals)
...
def latin_hypercube_sampling( population_size: int, dimension: int, lower: Array, upper: Array, rng: np.random.Generator) -> Array:
    ...
    for coordinate in range(dimension):
        unit_samples = (
            rng.permutation(population_size) + rng.random(population_size)) / population_size
...
def validate_problem( lower: Array, upper: Array, population_size: int, max_evaluations: int) -> tuple[Array, Array]:
    ...
    if lower.ndim != 1 or lower.shape != upper.shape or np.any(upper <= lower):
        raise ValueError(
            "lower and upper must be one-dimensional arrays with upper > lower")
...
def quadratic_penalty(
    objective: Objective,
    point: Array,
    inequalities: Optional[Constraint],
    equalities: Optional[Constraint],
    inequality_weight: float,
    equality_weight: float,
    equality_tolerance: float) -> float:
    ...
    if inequalities is not None:
        violation = np.maximum(
            0.0,
            np.atleast_1d(inequalities(point)).astype(float))
...
def prepare_objective( objective: Objective, inequalities: Optional[Constraint], equalities: Optional[Constraint], inequality_weight: float, equality_weight: float,
    equality_tolerance: float) -> Objective:
...
def summarize_penetration_diagnostics( totals: dict[str, float]) -> dict[str, float]:
    ...
    return { **{key: float(value) for key, value in totals.items()},
        ...
        "clipping_rate": float( totals["clipped_rows"] / max(1.0, totals["instar_steps"])),
...
def build_result( evaluator: BudgetEvaluator, cycles: int, return_history: bool, diagnostics: Optional[dict[str, float]] = None) -> dict[str, Any]:
    ...
    if diagnostics is not None:
        result["penetration_diagnostics"] = (
            summarize_penetration_diagnostics(diagnostics))
...
def build_axial_transverse_frames( starts: Array, guides: Array, rng: np.random.Generator) -> tuple[Array, Array, Array]:
    ...
    fallback_axis /= ( np.linalg.norm(fallback_axis, axis=1, keepdims=True) + 1.0e-12)
    ...
    if np.any(invalid):
        ...
        first_norm[rows] = np.linalg.norm( replacement, axis=1, keepdims=True)
    ...
    if np.any(invalid):
        rows = np.flatnonzero(invalid)
        coordinates = np.argmin( axis[rows] ** 2 + transverse_u[rows] ** 2, axis=1)
        replacement = np.zeros((rows.size, dimension), dtype=float)
        replacement[np.arange(rows.size), coordinates] = 1.0
        replacement -= ( np.sum(replacement * axis[rows], axis=1, keepdims=True) * axis[rows])
        replacement -= ( np.sum(replacement * transverse_u[rows], axis=1, keepdims=True) * transverse_u[rows])
        second[rows] = replacement
        second_norm[rows] = np.linalg.norm( replacement, axis=1, keepdims=True)
...
def coherent_larval_penetration(
    ...
    orientation = np.where( rng.random((larvae.shape[0], 1)) < 0.5, -1.0, 1.0)

    instars = max( 1, int( math.ceil( config.instars_max * (1.0 - progress) ** config.instar_power - 1.0e-12)))

    axial_total = config.axial_end + ( config.axial_start - config.axial_end) * (1.0 - progress) ** config.axial_power
    ...
    initial_distance = np.linalg.norm( normalized_guides - normalized_larvae,axis=1, keepdims=True) / max(normalized_scale, 1.0e-12)
    ...
    for instar in range(1, instars + 1):
        phase = initial_phase + orientation * ( 2.0 * math.pi * config.turns * instar / instars)
        transverse_direction = (np.cos(phase) * transverse_u + np.sin(phase) * transverse_v)
        ...
        transverse_step = ( offset - previous_offset
            if use_transverse
            else np.zeros_like(offset))

        proposal = centerline + offset if use_transverse else centerline
        clipped = np.clip(proposal, 0.0, 1.0)
        clipped_rows += int(np.sum(np.any(np.abs(clipped - proposal) > 1.0e-12, axis=1)))
        axial_path += float( np.sum( np.linalg.norm(axial_step, axis=1) / max(normalized_scale, 1.0e-12)))
        transverse_path += float(np.sum( np.linalg.norm(transverse_step, axis=1)/ max(normalized_scale, 1.0e-12)))
...
__all__ = ["PenetrationConfig", "HNWSOConfig", "h_nwso_optimize"]
